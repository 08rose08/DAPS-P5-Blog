...
<main class="bg-black p-3 text-white-50">
    <form method="post" action="index.php?action=addPost" class="mx-lg-5">
        ...
        <div class="form-group">
            ...
            <input value="<?php if(!empty($post)){echo htmlspecialchars($post->title());}; ?>" type="text" name="title" class="form-control" id ="title" placeholder="Titre" required />
        </div>
        <div class="form-group">
            <label for="chapo">Chapô</label>
            <input value="<?php if(!empty($post)){echo htmlspecialchars($post->chapo());}; ?>" type="text" name="chapo" class="form-control" id ="chapo" placeholder="Chapô" required />
        </div>
        <div class="form-group">
